...
impl VadIter {
    pub fn new(silero: VoiceActivityDetector, params: VadParams) -> Self {
        Self {
            silero,
    ...
    pub fn process(&mut self, samples: &[f32]) -> Result<(), voice_activity_detector::Error> {
        self.reset_states();
        for audio_frame in samples.chunks_exact(self.params.frame_size_samples) {
            let speech_prob: f32 = self.silero.predict(audio_frame.to_vec());
